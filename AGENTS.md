...
- 通过 Codex CLI 自动执行模型任务时统一调用 `scripts/codex_task_router.py`：标签和推荐优先 Luna，高质量简化审核使用 Luna/medium，日常判题使用 Terra，完整推导、修复和争议题使用 Sol；输入经本地压缩后走 stdin，模型固定在只读沙箱内，最多自动升级一次，正式写库仍只能经过现有质量门。详细规则见 `MODEL_ROUTING.md`。
