...
- 数学、物理、化学三个项目的 `photo-preflight` 共用整机级 OCR 执行锁。多会话必须等待权威入口排队，禁止另建 OCR 脚本、改用项目私有锁或通过并行进程绕过；取得执行槽后会再次检查本项目内容哈希缓存。共享锁可用绝对路径环境变量 `LIZHAOLIN_OCR_SHARED_LOCK` 覆盖。
