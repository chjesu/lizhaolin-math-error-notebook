...
| RL-02 | 手机号小时/日限流 | 单号码跨设备、跨 IP 请求 | 达 5/小时或 10/日后停止发送，不能通过换设备/IP 绕过 |
...
| SMS-02 | 回执签名与幂等 | 正确、错误、过期签名；重复回调 | 仅正确签名写入；重复事件只有一条；重放被拒绝 |
...
4. 同一供应商回执事件只落库一次；重复回调不重复改变状态或预算。
...
- 短信模板、签名、回调验签和测试号码已配置，真实通道小流量验证成功；
...
| 供应商受理失败、回执失败、延迟 | 失败率 >5% 或 P95 >30 秒持续 5 分钟 | 10 分钟确认供应商状态 |
...
- 供应商原始回调只保留验签和争议所需最小字段；完整 payload 如必须保留，应加密并设置更短期限。
