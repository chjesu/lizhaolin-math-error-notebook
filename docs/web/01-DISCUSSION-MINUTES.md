...
1. 使用阿里云承载 Web 服务。
2. 应用和数据库需要物理分离。
3. 生产关系型数据库正式确定采用阿里云 RDS MySQL 8.0，不再评估 PostgreSQL 等替代关系型数据库。
4. Web 版必须包含 Agent 边界护栏。
5. 当前讨论应整理成标准项目管理文档，作为后续实施依据。
6. Web 版需要从首版开始考虑多用户，避免个人数据模型上线后再次破坏性重构。

## 3. 讨论结论

### 3.1 产品边界

- 第一阶段面向邀请制家庭租户，可同时服务多个家庭和多个学生。
- “多用户”不等于首版建设完整学校 SaaS：首版不做计费、开放注册、复杂组织树和客服系统。
- 学生端聚焦上传、判题、订正、复习和练习 PDF。
- 家长端聚焦任务、历史、纠错、难度调整和进度。
- 一个监护人可绑定多个学生；一个学生可在明确授权下绑定多个监护人。
...
- MySQL 是已确认的生产关系型数据库，不是待选技术；后续只需决定 RDS 实例系列、规格和高可用方式。
...
- MySQL 采用共享 schema；租户私有数据必须带 `tenant_id`，学生学习数据同时带 `student_id`。
- 已验证公共题库跨租户只读共享；用户错题、作答、推荐和文件不得跨租户共享。
...
| 首版建设计费型完整多租户 SaaS | 暂缓 | 首版只实现不可省略的数据隔离、账号关系和审计，不提前建设计费、客服和复杂组织能力。 |
| 每用户或每家庭独立数据库 | 否决 | 运维、迁移和公共题库复用成本高；共享 schema 加强制租户约束足以覆盖当前规模。 |
...
| 编号 | 事项 | 决策时点 |
|---|---|---|
| O-001 | 首批邀请租户、人数上限和邀请码生命周期 | Gate 0 |
...
| O-007 | 教师/班级角色进入 MVP 还是后续版本 | Gate 0 |
| O-008 | 未满十四周岁用户的监护人同意、数据导出和删除流程 | 身份模块实施前 |
...
6. 建立跨租户负向测试，并把当前李兆霖数据迁入一个种子家庭租户。
