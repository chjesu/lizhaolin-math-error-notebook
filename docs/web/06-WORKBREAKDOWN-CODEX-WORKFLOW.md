...
- 短信发送状态不明：先查询供应商回执，禁止直接重复发送。

## 8. Codex CLI 插件与模型路由

项目统一通过现有 `codex_task_router.py` 调用 Codex CLI，不允许业务代码自行拼接 CLI 参数或绕过 Schema。安装 profile：
...
```powershell
python -X utf8 -B .agents\skills\math-error-notebook\scripts\codex_task_router.py route `
...
```powershell
python -X utf8 -B .agents\skills\math-error-notebook\scripts\codex_task_router.py run `
  --task verify-simplified `
  --input data\audits\<batch>\manifest.json `
  --out data\audits\<batch>\codex-decisions.json `
  --json
```
...
| 等级 | 适用任务 | 默认模型 | 触发升级 |
|---|---|---|---|
| L0 纯本地 | 哈希、去重、格式、Schema、权限、限流计数、SymPy | 不调用模型 | 无；失败交确定性错误处理 |
| L1 低风险判断 | 标签候选、推荐初排、可信批次简化审核 | Luna low/medium | 图像、冲突、缺项、低置信度 |
| L2 常规理解 | 文字判题、一般视觉判题、错因和引导、普通代码任务 | Terra medium | 复杂图形、证明、多分类、答案冲突 |
| L3 高风险推理 | 完整验证、题目修复、复杂工作流、安全关键设计 | Sol high | 不再自动升级 |
| L4 争议裁决 | 争议题、关键架构/安全裁决 | Sol xhigh | 无法确定则人工复核 |
