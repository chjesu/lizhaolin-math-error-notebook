...
**艾宾浩斯遗忘曲线是这个错题本的核心学习原理。** 新形成的记忆在最初阶段遗忘最快，随后逐渐趋缓；因此，“订正一次”并不等于“已经掌握”。真正有效的错题学习，需要在记忆即将衰减时再次主动回忆，通过一次次成功提取巩固记忆，并逐步拉长复习间隔。

本项目不机械套用固定的“遗忘百分比”，而是把遗忘曲线落实为可追踪、能根据实际表现重排的间隔复习计划：

```mermaid
flowchart LR
    A["订正当天：理解第一处错误"] --> B["第1天：主动回忆"]
    B --> C["第2天：同类型巩固"]
    C --> D["第4天：同类型巩固"]
    D --> E["第7天：变式练习"]
    E --> F["第15天：变式练习"]
    ...
    H -->|正确| I["完成本轮或进入下一阶段"]
    H -->|错误| B
    H -->|部分正确| J["按1、3、7、15、30、45天重排"]
```

每次复习都要求学生重新提取方法，而不是浏览答案：阶段1—2侧重同难度巩固，阶段3—4进入变式，阶段5—6检验迁移。答错时重新启动复习周期；部分正确时按更适合补强的节奏重排；做对时则以实际完成日期为基准顺延后续阶段。这样，错题本保存的不是一张静态答案，而是一条对抗遗忘、最终走向稳定掌握的学习轨迹。
...
5. **按遗忘曲线安排间隔复习**：从同难度巩固逐步过渡到变式和略高难度迁移；每次结果决定继续、顺延或重新启动复习周期。
