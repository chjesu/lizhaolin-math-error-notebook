...
- 本项目 GUI 新任务默认使用 `.codex/config.toml` 中的 Luna/low，GUI 智能体只负责编排；判题、题目内容/标签判断、推荐复核、审核验证、修复、生成和争议裁决等一切需要模型判断的有边界任务，默认且必须调用 `.agents/skills/math-error-notebook/scripts/codex_task_router.py`。标签和推荐优先 Luna，高质量简化审核使用 Luna/medium，日常判题使用 Terra，完整推导、修复和争议题使用 Sol；风险或低置信度最多自动升级一次。DOCX/PDF 提取、结构预检、去重、事务写入等确定性步骤仍直接使用现有脚本，不调用模型。路由结果只读，正式写库仍只能经过现有质量门；仅当路由器不可用且已明确告知用户时，才允许当前 GUI 模型临时直接承担模型判断。详细规则见 `MODEL_ROUTING.md`。
