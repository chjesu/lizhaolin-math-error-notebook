...
- This project defaults the GUI orchestrator to Luna/low. For every bounded task that needs mathematical or visual judgment, call `scripts/codex_task_router.py`; it selects the model once from the task and explicit risk flags before execution. Deterministic extraction, preflight, validation, and database writes still go directly through the existing scripts without a model call.
