...
sequenceDiagram
    ...
    S->>A: POST /v1/auth/sms/callback（签名 + 时间戳）
    A->>D: 按 provider_message_id 幂等写回执
```
...
|---|---:|---:|---:|---|
| 手机号（HMAC 后） | 60 秒 | 5 | 10 | 防止单个号码被轰炸 |
...
- 短信回调验证供应商签名、时间戳和重放窗口；以 `provider + provider_message_id + event_type` 唯一约束实现幂等。失败、拒收、无效号码和退订状态纳入风险评分。
- 维护全局、租户和供应商三级小时/日预算。达到 80% 告警并强制 CAPTCHA，达到 100% 熔断新短信；已发挑战仍可验证。管理员恢复必须记录审计，自动恢复只能发生在新预算窗口。
- 供应商调用设短超时且不自动跨供应商重试，避免一次用户请求产生多条短信。只有“明确未受理”才允许用户重新请求；未知结果等待回执。
